...
Open Notebook supports OpenAI-compatible API endpoints across multiple AI modalities (language models, embeddings, and optional text-to-speech), giving you the flexibility to use popular tools like LM Studio, Text Generation WebUI, vLLM, and custom inference servers.
...
> **Looking for local-only options?** Pair Open Notebook with providers like LM Studio or vLLM for fully self-hosted AI generation.
