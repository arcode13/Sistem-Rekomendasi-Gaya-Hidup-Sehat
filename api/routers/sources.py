...
ALLOWED_UPLOAD_EXTENSIONS_DISPLAY = ", ".join(
    sorted(ext.upper() for ext in ALLOWED_UPLOAD_EXTENSIONS)
)
    ...
    if extension not in ALLOWED_UPLOAD_EXTENSIONS:
        raise HTTPException(
            ...
            detail=(
                ...
                f"{ALLOWED_UPLOAD_EXTENSIONS_DISPLAY}"
            ),
        )
...
async def save_uploaded_file(upload_file: UploadFile) -> str:
    ...
    _validate_upload_extension(upload_file.filename)
...
async def get_sources(
    ...
    try:
        ...
        if notebook_id:
            ...
            query = f"""
                SELECT id, asset, created, title, updated, topics, command, chat_include,
        ...
        for row in result:
            ...
            response_list.append(
                SourceListResponse(
                    ...
                    chat_include=row.get("chat_include") or "full",
...
async def create_source(
    ...
    try:
        ...
        if source_data.type == "upload":
            ...
            _validate_upload_extension(final_file_path)
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid source type. Must be upload or text",
            )
...
async def get_source(source_id: str):
    ...
    try:
        ...
        return SourceResponse(
            ...
            created=str(source.created),
            updated=str(source.updated),
            chat_include=source.chat_include or "full",
...
async def view_source_file(source_id: str):
    """View the original file in browser (for iframe embedding)."""
    try:
        ...
        logger.error(f"Error viewing file for source {source_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to view source file")
...
async def get_source_status(source_id: str):
    ...
    try:
        ...
        try:
            ...
            if status == "completed":
                message = "Pemrosesan sumber berhasil diselesaikan"
            elif status == "failed":
                message = "Pemrosesan sumber gagal"
            elif status == "running":
                message = "Pemrosesan sumber sedang berlangsung"
            elif status == "queued":
                message = "Pemrosesan sumber dalam antrian"
            elif status == "unknown":
                message = "Status pemrosesan sumber tidak diketahui"
            else:
                message = f"Status pemrosesan sumber: {status}"
...
async def update_source(source_id: str, source_update: SourceUpdate):
    ...
    try:
        ...
        if source_update.chat_include is not None:
            if source_update.chat_include not in ("off", "full"):
                raise HTTPException(status_code=400, detail="chat_include must be 'off' or 'full'")
            source.chat_include = source_update.chat_include
...
async def retry_source_processing(source_id: str):
    ...
    try:
        ...
        if source.asset and source.asset.file_path:
            ...
            # Treat sources without files as text entries
...
async def delete_source(source_id: str):
    ...
    try:
        ...
        if source.asset and source.asset.file_path:
            ...
            if resolved_path.startswith(safe_root) and os.path.exists(resolved_path):
                try:
                    os.unlink(resolved_path)
                    logger.info(f"Deleted file: {resolved_path}")
                except Exception as file_error:
                    logger.warning(f"Failed to delete file {resolved_path}: {str(file_error)}")

        await Notebook.update_timestamps_for_source(source_id)
        await source.delete()
