...
const STATUS_CONFIG = {
  new: {
    ...
    label: 'Memproses',
    description: 'Mempersiapkan untuk diproses'
  },
  queued: {
    icon: Clock,
    color: 'text-blue-600',
...
export function SourceCard({
  ...
  const title = source.title || 'Sumber Tanpa Judul'
      ...
      <CardContent className="px-3 py-1">
        ...
        <div className="flex items-start justify-between gap-3 mb-1">
          <div className="flex-1 min-w-0">
            ...
            {!isCompleted && (
              <div className="flex items-center gap-2 mb-2">
                <div className={cn(
                  ...
                  {statusLoading && shouldFetchStatus ? 'Memeriksa...' : statusConfig.label}
                </div>
              </div>
            )}
            ...
            <div className="mb-1.5">
              <Badge variant="outline" className="text-xs">
                Embedding: {source.embedded ? 'Ya' : 'Tidak'}
              </Badge>
              <div className="mt-1">
          ...
          <div className="flex items-center gap-2">
            {/* Created time - show before context toggle */}
            <span className="text-xs text-muted-foreground">
            ...
            {onDelete && (
              <Button
                variant="ghost"
                size="sm"
                className="h-8 w-8 p-0 transition-colors text-muted-foreground hover:bg-muted"
                onClick={(e) => {
                  e.stopPropagation()
                  handleDelete()
                }}
        ...
        {isProcessing && statusData?.processing_info?.progress && (
          <div className="mt-3 pt-2 border-t">
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs text-gray-600">Kemajuan</span>
