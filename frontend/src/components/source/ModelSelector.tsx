    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Override model default untuk sesi chat ini. Biarkan kosong untuk menggunakan default sistem.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-2">
            ...
            <Select value={selectedModel} onValueChange={setSelectedModel}>
              <SelectTrigger id="model">
                <SelectValue placeholder="Pilih model (atau gunakan default)" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="default">
                  <div className="flex items-center justify-between w-full">
                    <span>
                      {defaultModel ? `Default (${defaultModel.name})` : 'Default Sistem'}
                    </span>
          ...
          {selectedModel && selectedModel !== 'default' && (
            <div className="rounded-lg bg-muted p-3">
              <p className="text-sm text-muted-foreground">
                Sesi ini akan menggunakan <strong>{languageModels.find(m => m.id === selectedModel)?.name}</strong> alih-alih model default.
              </p>
            </div>
          )}
        </div>
