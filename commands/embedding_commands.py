...
async def embed_single_item_command(
    ...
    try:
        ...
        if input_data.item_type == "source":
            ...
            raise ValueError(
                f"Invalid item_type: {input_data.item_type}. Must be 'source'"
            )
...
async def collect_items_for_rebuild(
    ...
    Returns:
        Dict with keys: 'sources' containing lists of item IDs
    """
    items: Dict[str, List[str]] = {"sources": []}
...
async def rebuild_embeddings_command(
    ...
    """
    Rebuild embeddings for sources
    ...
    try:
        ...
        logger.info(f"Include: sources={input_data.include_sources}")
        logger.info("=" * 60)
        ...
        processing_time = time.time() - start_time
        processed_items = sources_processed
